    ...
    folders.forEach((task) => {
        ...
        if (config[task].hasOwnProperty("modules")) {
            config[task].modules.forEach((part) => {
                if (part.indexOf('web/app/themes') > 0 || part == 'web/app/themes') {
                    part = part.replace('web/app/themes', 'web/app/themes/woody-theme');
                }
                var src = path.resolve(config.core, part.replace('WP_SITE_KEY', mode.site_key), '**/', config[task].extensions);
                console.log('WATCH INCLUDE (' + task + ')', src);
                gulp.watch([src, '!' + config.dist], gulp.series(task, 'size'));
            });

            var src = path.resolve(config.core, config[task].src.replace('WP_SITE_KEY', mode.site_key), '**/', config[task].extensions);
            console.log('WATCH INCLUDE (' + task + ')', src);
            gulp.watch([src, '!' + config.dist], gulp.series(task, 'size'));
        } else if (Array.isArray(config[task].src)) {
            config[task].src.forEach(function(part) {
                console.log('WATCH INCLUDE (' + task + ')', part);
                gulp.watch([part, '!' + config.dist], gulp.series(task, 'size'));
            });
        } else {
            console.log('WATCH INCLUDE (' + task + ')', config[task].src);
            gulp.watch([config[task].src, '!' + config.dist], gulp.series(task, 'size'));
        }
    });
});
