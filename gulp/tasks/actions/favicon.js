...
config.favicon.src.forEach(function(part, index, array) {
    array[index] = path.resolve(config.core, part.replace('WP_SITE_KEY', mode.site_key), config.favicon.extensions);
});
...
const iconsVariants = (file, done) => {
    ...
    done(null, [icon192, icon180, icon167, icon152, icon128, icon120, icon64, icon32, icon16])
}
...
gulp.task('touch-icons', () => {
    return gulp
        ...
        .pipe(changed(
            path.resolve(config.dist, config.favicon.dist), { hasChanged: changed.compareContents }
        ))
...
gulp.task('favicon-icons', () => {
    return gulp
        ...
        .pipe(ico('favicon.ico', { resize: true, sizes: [16, 32, 64, 128, 192] }))
        .pipe(gulp.dest(path.resolve(config.dist, config.favicon.dist)));
});
