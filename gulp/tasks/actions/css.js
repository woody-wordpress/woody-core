...
config.css.modules.forEach(function(part, index, array) {
    array[index] = path.resolve(config.core, part.replace('WP_SITE_KEY', mode.site_key));
});
...
config.css.entry.forEach(entry => {
    gulp.task(entry, () => {
        return gulp
            .src(path.resolve(config.core, config.css.src.replace('WP_SITE_KEY', mode.site_key), entry))
            .pipe(gulpif(mode.env == 'dev', sourcemaps.init()))
            .pipe(gulpif(mode.env == 'dev', plumber({
            ...
            .pipe(gulpif(mode.env != 'dev', autoprefixer({
                overrideBrowserslist: ['last 2 versions and > 0.5%'],
                cascade: false
            })))
            .pipe(gulpif(mode.env != 'dev', cleanCSS({
                level: 2
            })))
