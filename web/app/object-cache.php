...
class WP_Object_Cache
{
    ...
    /**
     * List of keys not saved to Memcached.
     *
     * @var array
     */
    public $no_mc_keys = array();
    ...
    public function __construct($persistent_id = null)
    {
        ...
        } elseif (!empty(MEMCACHED_HOST) && !empty(MEMCACHED_PORT)) {
            $memcached_servers[] = MEMCACHED_HOST . ':' . MEMCACHED_PORT;
        }

        $this->servers = [];
        foreach ($memcached_servers as $server) {
            $this->servers[] = explode(':', $server);
        }
    ...
    public function add($key, $value, $group = 'default', $expiration = 0, $server_key = '', $byKey = false)
    {
        ...
        if (in_array($group, $this->no_mc_groups) || $no_mc_key) {
        ...
        $this->debug('add', $derived_key, $value);
        return $result;
    }
    ...
    public function append($key, $value, $group = 'default', $server_key = '', $byKey = false)
    {
        ...
        $this->debug('append', $derived_key, $combined);
        return $result;
    }
    ...
    public function cas($cas_token, $key, $value, $group = 'default', $expiration = 0, $server_key = '', $byKey = false)
    {
        ...
        $this->debug('cas', $derived_key, $value);
        return $result;
    }
    ...
    public function decrement($key, $offset = 1, $group = 'default')
    {
        ...
        $this->debug('decrement', $derived_key);
        return $result;
    }
    ...
    public function get($key, $group = 'default', $force = false, &$found = null, $server_key = '', $byKey = false, $cache_cb = null, &$cas_token = null)
    {
        ...
        $this->debug('get', $derived_key, $value);
    ...
    public function increment($key, $offset = 1, $group = 'default')
    {
        ...
        $this->debug('increment', $derived_key);
        return $result;
    }
    ...
    public function prepend($key, $value, $group = 'default', $server_key = '', $byKey = false)
    {
        ...
        $this->debug('prepend', $derived_key, $combined);
        return $result;
    }
    ...
    public function replace($key, $value, $group = 'default', $expiration = 0, $server_key = '', $byKey = false)
    {
        ...
        $this->debug('replace', $derived_key, $value);
        return $result;
    }
    ...
    public function set($key, $value, $group = 'default', $expiration = 0, $server_key = '', $byKey = false)
    {
        ...
        $this->debug('set', $derived_key, $value);
        return $result;
    }
    ...
    public function setMulti($items, $groups = 'default', $expiration = 0, $server_key = '', $byKey = false)
    {
        ...
        $this->debug('setMulti', $derived_keys, $derived_items);
        return $result;
    }
    ...
    public function debug($action, $derived_key, $value = null)
    {
        if ($this->debug_mode) {
            if ($action == 'get') {
                return;
                $log_file = 'object-cache-read';
            } else {
                $log_file = 'object-cache-write';
            }
            $log_file = sprintf(__DIR__ . '/../../logs/%s.log', $log_file);
            ...
            error_log('[' . date('d-m-Y H:i:s') . '] ' . strtoupper($action) . ' : ' . $log . "\n", 3, $log_file);
        }
    }
}
